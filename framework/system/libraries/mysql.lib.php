...
class Mysql {
	...
	{   
		...
		{	
			$host  = $db2Key["mysqlhost"];
			$port = $db2Key["mysqlport"];
		    $user  = $db2Key["mysqluse"];
		    $password = $db2Key["mysqlpasswprd"];		  
		    $dbNam = $db2Key["db"];
		} 
    ...
    public function select($table, $select = '*', $where = '', $prepare = array(), $condition = '') {
       ...
       //__log_message("select:::".$where);
        
        $sql = " SELECT {$select} FROM {$table} $where {$condition}";
        
        //__log_message("select:::".$sql);
     ...
     * @return bool on execute result
     */
    public function insert($table, $array = array(),$addItion=NULL) {
        $sql = " INSERT INTO {$table} ";
        $fields = array_keys($array);
        $values = array_values($array);
        $condition = array_fill(1, count($fields), '?');
        $sql .= "(`" . implode('`,`', $fields) . "`) VALUES (" . implode(',', $condition) . ")";
        return $this->_query($sql.$addItion, $values);
    }
     ...
     */
    public function insertBatch($table,$fields,$data=array(),$addItion=NULL){
        if(empty($fields))
            return false;
    ...
    private function _query($sql, $prepare = array()) {
        ...
        if ($this->dbconfig['table_alias']) {
        	...
        	//__log_message("_query alias=".$this->dbconfig['table_alias']);
        	//__log_message("_query table=".$this->dbconfig['table']);
        	//__log_message("_query sql=".$sql);
        	
            $sql = preg_replace(
                    '/(from|update|insert|replace into|insert into|delete|delete from)\s+' .
					$this->dbconfig['table_alias'] . ' ?/is', ' \1 ' . $this->dbconfig['table'] . ' ', $sql, 1
            );
        }
       ...
       // __log_message("ban mysqli query1".$sql);
        $statement = $this->dbh->prepare($sql);		
        
        //__log_message("ban mysqli query2".$sql);
        ...
        if (!is_object($statement)) 
        {
            ...
            __log_message($sql, 'db');
           // __log_message($this->dbh->errorInfo(), 'state');
            return false;
        }
        //var_dump($prepare);
       ...
       // __log_message("statement is not prepare".$prepare);
