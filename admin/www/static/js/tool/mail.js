$(function() {
    ...
    $("#savePerBtn").click(function() {
    	...
    	var server = $("#savePerMailForm input[name='server']").val();
    	if(server=='' || server ==0 || server ==null ){
    		alert('请输入区服!');
    		return false;
    	}
    	var roleid = $("#savePerMailForm input[name='roleid']").val();
    	if(roleid=='' || roleid ==0 || roleid ==null ){
    		alert('请输入角色ID!');
    		return false;
    	}
    	var mailtitle = $("#savePerMailForm input[name='mailtitle']").val();
    	if(mailtitle=='' || mailtitle ==0 || mailtitle ==null ){
    		alert('请输入邮件标题!');    		
    		return false;
    	}
    	var context = $("#savePerMailForm textarea[name='context']").val();
    	 
    	if(context==='' || context == 0 || context ==null ){
    		alert('请输入邮件内容!');
    		return false;
    	}
