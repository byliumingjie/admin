...
function getServerInfo($type,$getRoledb)
{
	...
	{		
		...
		_log("getServerInfo  false data null ::".$sql,'face-upload-log');
		return false;
	} 
	_log("getServerInfo  false rowcount ::".$sql.'type::'.$type,'face-upload-log');
	return false;
} 
// CURL数据通讯
		...
		$contentType ='application/json;', $timeout = 30, $proxy = false)
{
	...
	else if('GET' === strtoupper($method))
	{
		...
		_log("send_request url::".$real_url. urldecode($urldata),'face-upload-log');
...
{ 
		...
		_log('str info data'.$datastr,'face-upload-log');
		
		$md5data = md5(strtoupper($key.$request.$key));
		
		_log("md5 data::".$md5data,'face-upload-log');
		...
		if(empty($platform))
		{
			...
			_log("platform info get false".json_encode($platform),'face-upload-log');
			return false;
		}
		$Header['url'] = 'http://'.$platform['platformhost'].
		':'.$platform['platformport'].'/'.$manager.''.$action.'/';
		_log("url data::".$Header['url'],'face-upload-log');

		
		$Header['request'] = $Inrequestjson.'/'.$md5data;
		
		_log("request data::".$Inrequestjson.$md5data,'face-upload-log');
...
function fileLoad($fales,$dataOut)
{
	...
	{   
		...
		*/
		...
		_log("files:::".json_encode($fales),'face-upload-log');
	...
	_log("datainfo files ::: false",'face-upload-log');
	return false;
}
...
function send($dataOut,$db,$logtrueinfo=null,$falseinfo=null)
	{
		...
		_log("send info dataInfo :".json_encode($dataOut),'face-upload-log');
		...
		$ret = send_request($inHeader['url'],$inHeader['request'],'gbk'); 
		 ...
		 _log("server return ret status ".$ret,'face-upload-log');
