...
class Menu_Service {
    ...
    public static function generateNavStructure($menutree) 
    {
        ...
        //对于参数调整默认指定一个
        //Session::get('platformPermit')
        $platformPermit = intval(Session::get('platformPermit'));
		//$_COOKIE['gzoneid'] =$platformPermit; 
       /* empty($platformPermit)
        ?
        Controller::prompt("你的账号所属平台是非法的或者为空！请联系管理员！",false)
        :"";*/
        ...
        foreach ($menutree as $obj) {
            if ($obj->root['t_pid'] == 0) {
            	...
            	// 如果需要指定平台的话那么可以在编辑的时候对targv这个参数在后台编辑里面做下编辑，
                // 然后在这个判断即可 ,目前默认都设置                
                $url = ($obj->root['t_controller'] == '#' || $obj->root['t_method'] == '#') 
                ? '#' : $obj->root['t_controller'] . '/' . $obj->root['t_method'] .(
                $obj->root['t_argv']?'?'.$obj->root['t_argv'].'='.$platformPermit:'');
                    ...
                    if (empty($obj->children)) {
                        ...
                        foreach ($obj->children as $child) {
                            if (empty($child->children)) {
                                $structure .= '<li>';
                                $structure .= '<a href="/' . $child->root['t_controller'] . '/' . $child->root['t_method'] . '" target="main-frame"><i class="icon icon-th-list"></i><span>' .
                                 $child->root['t_mname'] . ' </span></a>';
                                $structure . '</li>';
                            } else {                               
                                $structure .= '<li class="submenu">';
                                $structure .= '<a href="#' . $child->root['t_controller'] . '-' . $child->root['t_method']  .($child->root['t_argv']?'?'.$child->root['t_argv']:''). '" >
                                <i class="icon icon-th-list"></i><span>' . $child->root['t_mname'] . '</span></a>';
